...
Selvedge 当前以 `@maolaila1/selvedge` 这个 npm 包名发布，并暴露
`selvedge` 命令行入口。
...
2. 安装 pnpm 9.x。
3. 使用有权限发布 `@maolaila1` scope 的 npm 账号登录：
...
```sh
npm view @maolaila1/selvedge name version --json
```
...
cd ../selvedge-smoke
npm init -y
npm install ../selvedge/maolaila1-selvedge-0.1.0.tgz
...
```sh
npm view @maolaila1/selvedge name version dist-tags --json
mkdir ../selvedge-published-smoke
cd ../selvedge-published-smoke
npm init -y
npm install @maolaila1/selvedge
npx selvedge init
npx selvedge status
```
